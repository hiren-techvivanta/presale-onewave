...
const Register = () => {
  ...
  const countryObj = customList("countryCode", "{countryNameEn} (+{countryCallingCode})");
  const countries = Object.entries(countryObj).map(([code, name]) => ({ code, name }));
  ...
  const validateStepOne = () => {
    ...
    if (form.refEmail && !emailRegex.test(form.refEmail)) {
      newErrors.refEmail = "Invalid referral email.";
    }
  ...
  return (
    <>
      <div className="blur-bg py-3 px-4 d-flex justify-content-between align-items-center w-100 shadow-sm position-absolute top-0">
        ...
        <button className="btn btn-primary px-4" onClick={() => navigate("/signin")}>
          Sign In
        </button>
      </div>
      ...
      <div className="register-page position-relative vh-100">
        ...
        <div className="position-absolute translate-centrt w-100 mw-100 px-3">
          ...
          <div className="pt-2 px-3 pb-0">
            ...
            <p className="text-secondary mb-4">Create account today and start using Wave</p>
            </div>
            <hr className="my-1" />
          
            <form className="pt-2 pb-3 px-3" onSubmit={showPasswordFields ? handleSubmit : handleContinue}>
             {!showPasswordFields &&( <>
              ...
              <div className="mb-2">
                ...
                <div className="input-group">
                  <span className="input-group-text">
                    +{form.country ? callingCodeMap[form.country] : "___"}
                  </span>
                  <input
                    type="tel"
              ...
              <div className="mb-2">
                ...
                <input
                  type="email"
                  className={`form-control ${errors.refEmail ? "is-invalid" : ""}`}
                  placeholder="Referral Email"
                  name="refEmail"
                  value={form.refEmail}
                  onChange={handleChange}
                />
                {errors.refEmail && <div className="invalid-feedback">{errors.refEmail}</div>}
              </div>
             </>)}
